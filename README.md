...
- 默认关闭的 Android/iOS 推送、语音、原生播放器与手机只读查询扩展接口
...
公开版为 Android/iOS 伴侣应用预留了签名消息推送 webhook；语音、原生手机播放器和 AI 查询手机通过伴侣端或用户自建的 MCP 服务接入。这些原生扩展默认全部关闭，不包含任何真实设备权限或厂商账号；网页内置的一起听房间不依赖伴侣端。协议、示例和安全边界见 [移动端扩展接口](docs/MOBILE_EXTENSIONS.md)。
