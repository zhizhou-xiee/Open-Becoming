...
- 默认关闭的 Android/iOS 推送、语音、共听音乐与手机只读查询扩展接口
...
公开版为 Android/iOS 伴侣应用预留了签名消息推送 webhook；语音、一起听音乐和 AI 查询手机通过伴侣端或用户自建的 MCP 服务接入。四者默认全部关闭，不包含任何真实设备权限或厂商账号。协议、示例和安全边界见 [移动端扩展接口](docs/MOBILE_EXTENSIONS.md)。
