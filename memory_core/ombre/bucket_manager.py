...
class BucketManager:
    ...
    async def create(
        ...
        metadata = {
            ...
            "last_active": last_active or created or now_iso(),
            "activation_count": max(0, float(activation_count or 0)),
        }
        if owner_id:
        ...
        if enrichment_error:
            metadata["enrichment_error"] = enrichment_error
        if enriched_at:
        ...
        if pinned:
            ...
            if bucket_type != "permanent":
                metadata["pinned_from_type"] = bucket_type
        ...
        if model_valence is not None:
            metadata["model_valence"] = max(0.0, min(1.0, float(model_valence)))
    ...
    async def update(self, bucket_id: str, **kwargs) -> bool:
        ...
        更新桶的内容或元数据字段。
        """
        refresh_last_active = bool(kwargs.pop("_refresh_last_active", True))
        file_path = self._find_bucket_file(bucket_id)
        if not file_path:
            return False
        ...
        is_pinned = was_pinned or post.get("protected", False)
        if is_pinned:
        ...
        if "pinned" in kwargs:
            ...
            if kwargs["pinned"]:
                if not was_pinned:
                    post["pinned_from_type"] = post.get("type", "dynamic")
                post["importance"] = 10  # pinned → lock importance to 10
        if "digested" in kwargs:
            post["digested"] = bool(kwargs["digested"])
        if "model_valence" in kwargs and kwargs["model_valence"] is not None:
            post["model_valence"] = max(0.0, min(1.0, float(kwargs["model_valence"])))
        if "activation_count" in kwargs:
            post["activation_count"] = max(0, float(kwargs["activation_count"] or 0))
        if "created" in kwargs and kwargs["created"]:
            post["created"] = kwargs["created"]
        if "last_active" in kwargs and kwargs["last_active"]:
            post["last_active"] = kwargs["last_active"]
        for field in (
            "enrichment_status",
            "enrichment_error",
            "enriched_at",
        ...
        if refresh_last_active and "last_active" not in kwargs:
        ...
        elif "pinned" in kwargs and not kwargs["pinned"] and was_pinned:
            restored_type = post.get("pinned_from_type", "dynamic")
            if "pinned_from_type" in post:
                del post["pinned_from_type"]
            if restored_type != "permanent":
                post["type"] = restored_type
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(frontmatter.dumps(post))
                target_dir = self.feel_dir if restored_type == "feel" else self.dynamic_dir
