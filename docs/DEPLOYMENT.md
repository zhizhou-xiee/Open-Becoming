...
公开仓库为了保护原作者隐私，默认只写了 `User` 和 `Char 1`–`Char 6`。建议在第一次启动、导入记忆或建立正式数据库前，先在 `.env` 或部署平台的 Variables / Secrets 页面填写：
...
保存并重启后，这些全局名字会出现在单聊、群聊、朋友圈、记忆、额度、摘要、一起听和语音等功能中。再进入「更多 → 换毛期°人设编辑」，把六份示例人设改成真正的人设；默认人设会自动使用环境变量里的角色名和你的称呼。

请特别区分两件事：

- `USER_DISPLAY_NAME` 和 `NAME_CHAR1`–`NAME_CHAR6` 是可以安全修改的全局显示名。
- `user`、`char1`–`char6` 是数据库关联、记忆隔离、头像设置和工具路由使用的内部 ID。不要对整个仓库做全局替换，也不要修改 `CHARACTERS` 的键或 `domain`，否则旧数据可能找不到对应角色。

聊天页长按名称修改的是聊天备注，只适合临时昵称；它不会覆盖其他功能区的全局名字。修改完成后至少检查一次：单聊标题、群聊发言者、记忆页、饭饭喵额度、朋友圈作者和说说喵音色列表。如果仍看见 `Char`，先确认环境变量已保存到正在运行的服务，再重新部署或重启容器。
...
# 自定义 OpenAI-compatible 服务按需填写
CUSTOM_OPENAI_API_KEY=replace-with-your-provider-key
CUSTOM_OPENAI_BASE_URL=https://provider.example/v1
```

只需要配置实际会使用的供应商。它们不会写进 HTML、JavaScript、本地数据库，也不会通过角色配置接口返回给浏览器；前端只能看到“已配置／未配置”。浏览器只向你部署的 Open-Becoming 请求 `/api/chat`，再由服务器携带 API key 请求模型服务商。

部署后可在「更多 → 换毛期°人设编辑」分别给每个角色选择供应商和模型，也能单独设置“压缩与群聊摘要”的供应商。切换供应商或模型时，后端会先发起一条最短连通性测试，失败则不会覆盖原配置。`DeepSeek 官方` 默认使用 `https://api.deepseek.com/chat/completions`；自定义 OpenAI-compatible 地址既可以填写 API 根地址（例如 `https://provider.example/v1`），也可以直接填写完整的 `/chat/completions` 地址。

| 前端选项 | 后端环境变量 | 地址变量 | 备注 |
|---|---|---|---|
| OpenRouter | `OPENROUTER_API_KEY` | `OPENROUTER_BASE_URL`（可选） | 默认角色与摘要仍使用这一项 |
| Anthropic 官方 | `ANTHROPIC_API_KEY` | `ANTHROPIC_BASE_URL`（可选，需为完整 Messages 地址） | 默认 `char5` 使用 |
| DeepSeek 官方 | `DEEPSEEK_API_KEY` | `DEEPSEEK_BASE_URL`（可选） | 默认地址为 `https://api.deepseek.com` |
| 自定义 OpenAI-compatible | `CUSTOM_OPENAI_API_KEY` | `CUSTOM_OPENAI_BASE_URL` | 自托管且无需鉴权时另设 `CUSTOM_OPENAI_ALLOW_NO_KEY=true` |
...
语音 TTS/STT token 也是例外：它从「更多 → 说说喵°语音收发」提交到自己的后端并保存在 SQLite 中，读取配置时只返回“已配置”，不会把原文回显到浏览器。请务必通过 HTTPS 打开部署站点，并把数据库及备份当作密钥文件。语音默认关闭，完整配置见 [语音收发教程](VOICE.md)。
...
| 变量 | 内容 | 是否必须备份 |
|---|---|---:|
| `DB_PATH` | 对话、摘要、设置、动态、MCP 配置、语音配置与 TTS 音频 | 是 |
...
2. 用 `APP_PASSWORD` 登录，到「换毛期°人设编辑」确认计划使用的供应商显示“后端凭证已配置”，并分别测试每条已启用线路。
...
7. 在仓库根目录运行 `rg "OPENROUTER_API_KEY|ANTHROPIC_API_KEY|DEEPSEEK_API_KEY|CUSTOM_OPENAI_API_KEY|NETEASE_MUSIC_U" static`；正常应没有结果。
